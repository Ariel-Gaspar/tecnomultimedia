class Principal {
  ...
  void cambiarPantallaBoton() {
    ...
    if (pantallaActual==1) {
      if (pantalla[index].botonSiguiente.botonPress(botonx, height-50, 100, 40)) {
        index++;
        
      }else if (index==9){
        if (pantalla[index].botonDesicion.botonPress(botonx,50, 100, 40)) {
          pantallaActual=6;
          index=0;
    ...
    if (pantallaActual==6){
      ...
      if (historiaAlternativa[index].botonSiguiente.botonPress(botonx, height-50, 100, 40)) {
        index++;
