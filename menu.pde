class Menu{
  ...
  Menu(){
    ...
    botonCreditos =new Boton(x-50, height/2+alto*4, ancho, alto);
    botonAtras= new Boton(width/2, height/2+alto*4, ancho, alto);
  ...
  void apretar(){
    if (botonEmpezar.botonPress(x, y2, 150, alto)){
      id=1;
    }else if (botonInstrucciones.botonPress(x, y2+alto+alto, ancho, alto)){
      id=2;
      
    }else if (botonCreditos.botonPress(width/2-50, height/2+(alto*4), ancho, alto)){
      id=3;
    } 
